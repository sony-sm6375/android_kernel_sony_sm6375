			   ...
			   unsigned char *buf, unsigned int len)
{
	...
	ret = i2c_master_send(aw8622x->i2c, &reg_addr, AW_I2C_BYTE_ONE);
	if (ret < 0) {
		aw_dev_err("%s: couldn't send addr:0x%02x, ret=%d\n",
			   __func__, reg_addr, ret);
		return ret;
	}
	ret = i2c_master_recv(aw8622x->i2c, buf, len);
	if (ret != len) {
				...
				size_t count)
{
	...
	if (aw8622x ==NULL)
		goto error;
	rc = sscanf(buf, "0x%02X", &val);
	if (rc == 0) {
		aw8622x->f0_cali_data = val;
	} else {
		 pr_err("%s:%u: error calibrtion data\n",__func__, __LINE__);
	 	goto error;
	}
	mutex_lock(&aw8622x->lock);
				 ...
				 struct device_attribute *attr, char *buf)
{
	...
	switch (aw8622x->gain) {
		case 0x80:
			len = scnprintf(buf, PAGE_SIZE, "100%% strength\n");
		break;
		case 0x60:
			len = scnprintf(buf, PAGE_SIZE, "75%% strength\n");
		break;
		case 0x40:
			len = scnprintf(buf, PAGE_SIZE, "50%% strength\n");
		break;
		#if 0
		case 0x20:
			len = scnprintf(buf, PAGE_SIZE, "25%% strength\n");
		break;
		#endif
		default:
			len = scnprintf(buf, PAGE_SIZE, "err gain!\n");
		break;
			}
	return len;
