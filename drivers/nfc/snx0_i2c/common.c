...
#ifdef HW_COLD_RESET
	nfc_gpio->cld_rst = of_get_named_gpio(np, DTS_CLD_RST_GPIO_STR, 0);
	if (!gpio_is_valid(nfc_gpio->cld_rst)) {
		pr_warn("%s: cld_rst gpio invalid %d\n", __func__,
			nfc_gpio->cld_rst);
	}
	pr_info("%s: %d, %d, %d, %d\n", __func__, nfc_gpio->irq, nfc_gpio->ven,
		nfc_gpio->dwl_req, nfc_gpio->cld_rst);
#else
	pr_info("%s: %d, %d, %d\n", __func__, nfc_gpio->irq, nfc_gpio->ven,
		nfc_gpio->dwl_req);
#endif
...
#ifdef HW_COLD_RESET
	if (gpio_get_value(nfc_gpio->cld_rst)) {
		pr_err("%s: hardware cold reset in-progress\n", __func__);
		return -EBUSY;
	}
#endif
